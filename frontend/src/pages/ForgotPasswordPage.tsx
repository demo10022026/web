...
export default function ForgotPasswordPage() {
  ...
  const handleSendOtp = async ({ email: e }: { email: string }) => {
    setLoading(true)
    try {
  ...
  const handleResetPassword = async ({ newPassword }: { newPassword: string; confirmPassword: string }) => {
    setLoading(true)
    try {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        ...
        {step === 'newPassword' && (
          <>
            ...
            <form onSubmit={pwForm.handleSubmit(handleResetPassword as any)} className="space-y-4">
