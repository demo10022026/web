...
import {
  ...
  Minus, Plus, Heart, Share2, Shield, Truck
} from 'lucide-react'
...
export default function ProductDetailPage() {
  ...
  if (!product) return (
    <div className="text-center py-20 text-gray-400">Sản phẩm không tồn tại</div>
  )

  const allImages = [product.thumbnailUrl, ...product.images].filter(Boolean)
  ...
  const handleBuyNow = () => {
    ...
    if (!selectedVariant) { toast.error('Vui lòng chọn phân loại'); return }
    if (stock === 0) { toast.error('Sản phẩm đã hết hàng'); return }
    // TODO: navigate to checkout (Sprint 3)
              ...
              <div className="flex gap-3">
                <button onClick={handleAddToCart}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3
                             border-2 border-orange-500 text-orange-500 font-semibold rounded-xl
